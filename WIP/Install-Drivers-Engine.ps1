...
#
# Run from Win+R in audit mode (single command, no prerequisites):
...
#
# Headless / scripted use (unchanged from 1.x):
...
#   powershell -ExecutionPolicy Bypass -File Install-Drivers-auto.ps1 -Manufacturer Microsoft -Model "Surface Laptop 4" -TestMode
#   powershell -ExecutionPolicy Bypass -File Install-Drivers-auto.ps1 -Silent -NoAnalytics
#
# Parameters: identical to 1.x - see -Manufacturer/-Model/-MachineType/-DriverRoot/
#   -Headless/-Silent/-TestMode/-Diagnostic/-NoAnalytics/-MaxParallelDownloads/
#   -SkipInstall/-SkipCleanup/-PromptWindowsUpdate.
#
# Supports: Dell, HP, Lenovo, Microsoft (Surface)
#
# Output files written to %USERPROFILE%\Downloads\ (timestamped, one set per run):
#   DriverInstaller_<ts>.log / .events.json / .analytics.json / .report.html
#
# ZERO-PREREQUISITE / CLEAN-MACHINE GUARANTEE
#   - Uses only in-box Windows components: WPF (PresentationFramework, in .NET
#     Framework on every Win10/11), WinForms (in-box; loaded only so legacy
#     DoEvents() calls in the preserved engine are inert no-ops and for the
#     native confirm/restart dialogs), curl.exe and expand.exe (ship in
#     Windows 10 1803+ / 11), pnputil (in-box).
#   - The portable 7-Zip helper (Install-7Zip) is downloaded to a temp folder
#     and removed by Remove-7Zip at end-of-run. Nothing is installed to
#     Program Files, no MSI, no PATH/registry changes.
#   - C:\DRIVERS (or -DriverRoot) is deleted at the end unless -SkipCleanup.
#   - powercfg/tzutil/w32tm calls match 1.x behaviour (audit-mode imaging).
#
# ============================ CHANGELOG ============================
# v2.0.0 - UI ENGINE REWRITE: WinForms -> WPF, single-threaded -> background
#          runspace + Dispatcher. This is a UI/threading change only; ALL
#          vendor logic (Dell CatalogPC, HP HPIA reference catalog, Lenovo
#          Recipe Card + consumer-catalog DetectInstall evaluator, Microsoft
#          Surface Download Center + v1.13.0 CPU-variant selection), the curl
#          download engine (serial + parallel, stall/cancel semantics), the
#          3-tier extraction fallback, pnputil bind-marker parsing, analytics
#          (Sheets webhook + local JSON + NDJSON event log), the HTML report,
#          and the v1.13.1 early serial capture are carried over UNCHANGED
#          from 1.13.1 (lines preserved verbatim; only $script:CancelRequested
#          was renamed to a synchronized $sync.Cancel so the WPF Cancel
#          button can signal the worker thread across the runspace boundary).
#          (1) Modern WPF UI: single dark card, Segoe UI, accent progress for
#              Download / Extract+Install / Overall, monospace live log,
#              status pill, Cancel + Run, Sound + Auto-scroll toggles.
#          (2) The entire install pipeline runs on a dedicated STA background
#              runspace. The UI thread only runs a ~90ms DispatcherTimer that
#              drains a synchronized log queue and mirrors $sync state onto
#              the WPF controls, so the window never freezes during multi-GB
#              downloads or long extractions (the core reason for the port).
#          (3) Legacy WinForms helper functions (Log/SetProgress/SetDownload/
#              SetExtract/Step-*/Stop-*/Test-Cancelled/Set-Button*/Play-Sound/
#              Open-WindowsUpdate) reimplemented against $sync with IDENTICAL
#              names and signatures, so the preserved engine/vendor code calls
#              them unmodified. Direct control pokes left in the preserved
#              code ($exBar.Style, $exSpinnerLabel.Text, $form.Text, etc.) are
#              satisfied by lightweight shim objects that write into $sync.
#          (4) Headless/-Silent path unchanged: no WPF, pipeline runs in-proc
#              exactly as 1.x (helpers short-circuit UI work when -Headless).
# v1.13.1 - Analytics: serial captured on every exit path (carried over).
# v1.13.0 - Surface: CPU-aware driver pack selection for dual-CPU models
#           (Surface Laptop 3/4 Intel vs AMD) (carried over).
#   ...earlier 1.x history retained in repo wiki / 1.13.1 header...
# ==================================================================
...
param(
    ...
    [switch]$SkipInstall,
    [switch]$SkipCleanup,
    [bool]$PromptWindowsUpdate = $true
)

# Auto-enable headless when any override param is passed (same rule as 1.x).
if ($Manufacturer -or $Model -or $MachineType -or $DriverRoot -ne "C:\DRIVERS" `
    -or $SkipInstall -or $SkipCleanup -or $Silent -or $TestMode -or $Diagnostic `
    -or $NoAnalytics -or $PromptWindowsUpdate -eq $false) {
    $Headless = $true
}
...
# In-box assemblies only. WPF for the new UI; WinForms stays loaded so the
# preserved engine's [System.Windows.Forms.Application]::DoEvents() calls are
# harmless near-no-ops on the worker thread, and the native confirm/restart
# MessageBox dialogs in the preserved Start-Install keep working.
...
# Power / clock prep (unchanged from 1.x; matters for audit-mode imaging).
powercfg /change standby-timeout-ac 0 | Out-Null
powercfg /change monitor-timeout-ac 0 | Out-Null
tzutil /s "E. Australia Standard Time" 2>$null
Start-Service w32tm -ErrorAction SilentlyContinue
w32tm /resync /force 2>$null | Out-Null

# Log file setup (same four sibling files, same naming as 1.x).
...
# Everything the preserved engine/vendor code expects at script scope is
# passed to the worker runspace through this single context bag.
$Ctx = @{
    ...
    NoAnalytics          = [bool]$NoAnalytics
    MaxParallelDownloads = $MaxParallelDownloads
    SkipInstall          = [bool]$SkipInstall
    SkipCleanup          = [bool]$SkipCleanup
    PromptWindowsUpdate  = $PromptWindowsUpdate
}

# Synchronized state shared between the worker runspace and the UI thread.
$sync = [hashtable]::Synchronized(@{})
$sync.Cancel        = $false
$sync.Finished      = $false
$sync.Running       = $false
$sync.SoundEnabled  = $true
$sync.AutoScroll    = $true
$sync.CloseWindow   = $false
$sync.Progress      = 0
$sync.Dl            = @{ Pct = 0;  Label = "Waiting...";  Mode = "idle"; Spin = "";  Dot = "idle" }
$sync.Ex            = @{ Pct = 0;  Label = "Waiting...";  Mode = "idle"; Spin = "";  Dot = "idle" }
$sync.Ov            = @{ Spin = ""; Dot = "idle" }
$sync.HeaderText    = "Extract & install"
$sync.Title         = "Driver Installer  v$SCRIPT_VERSION"
$sync.Subtitle      = "Detecting hardware..."
$sync.StatusPill    = "Idle"
$sync.LogQueue      = [System.Collections.Queue]::Synchronized([System.Collections.Queue]::new())

# =============================================================
# WORKER  -  runs on a dedicated STA background runspace (GUI mode)
#            or in-process on the main thread (-Headless / -Silent).
# Contains: Ctx unpack, UI shim objects, reimplemented UI helpers
# (identical names/signatures to 1.x), then the VERBATIM preserved
# engine + vendor + Start-Install, then the Start-Install call.
# =============================================================
$WorkerBody = {
    param($sync, $Ctx)

    # ---- Re-establish everything the preserved code expects at script scope
    $script:sync          = $sync
    $SCRIPT_VERSION       = $Ctx.Version
    $LogFile              = $Ctx.LogFile
    $EventsLogFile        = $Ctx.EventsLogFile
    $AnalyticsFile        = $Ctx.AnalyticsFile
    $ReportFile           = $Ctx.ReportFile
    $SpinnerFrames        = $Ctx.SpinnerFrames
    $Manufacturer         = $Ctx.Manufacturer
    $Model                = $Ctx.Model
    $MachineType          = $Ctx.MachineType
    $DriverRoot           = $Ctx.DriverRoot
    $Headless             = $Ctx.Headless
    $Silent               = $Ctx.Silent
    $TestMode             = $Ctx.TestMode
    $Diagnostic           = $Ctx.Diagnostic
    $NoAnalytics          = $Ctx.NoAnalytics
    $MaxParallelDownloads = $Ctx.MaxParallelDownloads
    $SkipInstall          = $Ctx.SkipInstall
    $SkipCleanup          = $Ctx.SkipCleanup
    $PromptWindowsUpdate  = $Ctx.PromptWindowsUpdate
    ...
    $script:MaxParallelDownloads = $MaxParallelDownloads
    $script:AutoScroll = $true
    $script:SpinnerIndex = 0
    $script:7zInstalled = $false

    # WinForms is needed in THIS runspace for the preserved Start-Install's
    # native confirm/restart MessageBox dialogs and Clipboard.SetText, and so
    # the inert DoEvents() calls in the preserved engine resolve.
    Add-Type -AssemblyName System.Windows.Forms -ErrorAction SilentlyContinue
    Add-Type -AssemblyName System.Drawing       -ErrorAction SilentlyContinue
    Add-Type -AssemblyName System.IO.Compression.FileSystem -ErrorAction SilentlyContinue

    # ---- UI SHIM OBJECTS ------------------------------------------------
    # The preserved engine/vendor/Start-Install code pokes a handful of
    # WinForms controls directly. These shims expose the same property names
    # but write into $sync, so that code runs byte-for-byte unmodified.
    function New-ShimLabel {
        param([string]$Slot, [string]$Field)
        $o = [pscustomobject]@{ __slot = $Slot; __field = $Field }
        $o | Add-Member -MemberType ScriptProperty -Name Text `
            -Value     { $script:sync[$this.__slot][$this.__field] } `
            -SecondValue {
                param($v)
                $h = $script:sync[$this.__slot]; $h[$this.__field] = "$v"
                $script:sync[$this.__slot] = $h
            }
        $o
    }
    $exHeaderLabel       = [pscustomobject]@{}
    $exHeaderLabel       | Add-Member ScriptProperty Text `
        { $script:sync.HeaderText } { param($v) $script:sync.HeaderText = "$v" }

    $exSpinnerLabel      = New-ShimLabel 'Ex' 'Spin'
    $dlSpinnerLabel      = New-ShimLabel 'Dl' 'Spin'
    $overallSpinnerLabel = New-ShimLabel 'Ov' 'Spin'

    $subtitle = [pscustomobject]@{}
    $subtitle | Add-Member ScriptProperty Text `
        { $script:sync.Subtitle } { param($v) $script:sync.Subtitle = "$v" }

    $form = [pscustomobject]@{}
    $form | Add-Member ScriptProperty Text `
        { $script:sync.Title } { param($v) $script:sync.Title = "$v" }
    $form | Add-Member ScriptMethod Close { $script:sync.CloseWindow = $true }

    # $exBar shim: maps .Style / .Value / .MarqueeAnimationSpeed onto $sync.Ex.
    $exBar = [pscustomobject]@{}
    $exBar | Add-Member ScriptProperty Style `
        { $h=$script:sync.Ex; if ($h.Mode -eq 'marquee') {'Marquee'} else {'Continuous'} } `
        { param($v) $h=$script:sync.Ex
          $h.Mode = if ("$v" -eq 'Marquee') {'marquee'} else {'continuous'}
          $script:sync.Ex = $h }
    $exBar | Add-Member ScriptProperty Value `
        { $script:sync.Ex.Pct } `
        { param($v) $h=$script:sync.Ex; $h.Pct=[int]$v; $h.Mode='continuous'; $script:sync.Ex=$h }
    $exBar | Add-Member ScriptProperty MarqueeAnimationSpeed { 30 } { param($v) }

    # ---- REIMPLEMENTED UI HELPERS (same names/signatures as 1.x) ---------
    function Log {
        param(
        ...
        $now  = Get-Date
        $line = "[$($now.ToString('HH:mm:ss'))] $msg"

        if (-not $script:Silent) {
            if ($script:Headless) {
                Write-Host $line
            } else {
                try { $script:sync.LogQueue.Enqueue($line) } catch {}
            }
        }
        ...
        try {
            ...
            Add-Content -Path $EventsLogFile -Value ($evt | ConvertTo-Json -Compress -Depth 4) -Encoding UTF8
        } catch {}
    }
    ...
    function SetProgress($val) {
        $script:sync.Progress = [math]::Min([math]::Max([int]$val, 0), 100)
    }

    function SetDownload {
        param([int]$Pct, [string]$Label)
        $h = $script:sync.Dl
        if ($Pct -ge 100)      { $h.Mode='continuous'; $h.Pct=100 }
        elseif ($Pct -le 0)    { $h.Mode='marquee' }
        else                   { $h.Mode='continuous'; $h.Pct=[math]::Min($Pct,99) }
        if ($PSBoundParameters.ContainsKey('Label')) { $h.Label = $Label }
        $script:sync.Dl = $h
    }
    ...
    function SetExtract {
        ...
        $h = $script:sync.Ex
        if ($Pct -ge 100)   { $h.Mode='continuous'; $h.Pct=100 }
        elseif ($Pct -lt 0) { $h.Mode='marquee' }
        else                { $h.Mode='continuous'; $h.Pct=[math]::Min($Pct,99) }
        if ($PSBoundParameters.ContainsKey('Label')) { $h.Label = $Label }
        $script:sync.Ex = $h
    }
    ...
    function Step-DlSpinner {
        ...
        $h=$script:sync.Dl; $h.Spin=$SpinnerFrames[$script:SpinnerIndex]; $h.Dot='work'; $script:sync.Dl=$h
    }
    function Stop-DlSpinner {
        param([bool]$Success = $true)
        $h=$script:sync.Dl; $h.Spin=$(if($Success){'OK'}else{'X'}); $h.Dot=$(if($Success){'ok'}else{'err'}); $script:sync.Dl=$h
    }
    function Step-ExSpinner {
        $script:SpinnerIndex = ($script:SpinnerIndex + 1) % $SpinnerFrames.Count
        $h=$script:sync.Ex; $h.Spin=$SpinnerFrames[$script:SpinnerIndex]; $h.Dot='work'; $script:sync.Ex=$h
    }
    function Stop-ExSpinner {
        param([bool]$Success = $true)
        $h=$script:sync.Ex; $h.Spin=$(if($Success){'OK'}else{'X'}); $h.Dot=$(if($Success){'ok'}else{'err'}); $script:sync.Ex=$h
    }
    function Step-OverallSpinner {
        $script:SpinnerIndex = ($script:SpinnerIndex + 1) % $SpinnerFrames.Count
        $h=$script:sync.Ov; $h.Spin=$SpinnerFrames[$script:SpinnerIndex]; $h.Dot='work'; $script:sync.Ov=$h
    }
    function Stop-OverallSpinner {
        param([bool]$Success = $true)
        $h=$script:sync.Ov; $h.Spin=$(if($Success){'OK'}else{'X'}); $h.Dot=$(if($Success){'ok'}else{'err'}); $script:sync.Ov=$h
    }
    function Step-AllSpinners {
        $script:SpinnerIndex = ($script:SpinnerIndex + 1) % $SpinnerFrames.Count
        $f = $SpinnerFrames[$script:SpinnerIndex]
        $d=$script:sync.Dl; $d.Spin=$f; $d.Dot='work'; $script:sync.Dl=$d
        $e=$script:sync.Ex; $e.Spin=$f; $e.Dot='work'; $script:sync.Ex=$e
        $o=$script:sync.Ov; $o.Spin=$f; $o.Dot='work'; $script:sync.Ov=$o
    }

    function Set-ButtonRunning {
        $script:sync.Running    = $true
        $script:sync.StatusPill = "Running"
        $d=$script:sync.Dl; $d.Dot='idle'; $script:sync.Dl=$d
        $e=$script:sync.Ex; $e.Dot='idle'; $script:sync.Ex=$e
        $o=$script:sync.Ov; $o.Dot='idle'; $script:sync.Ov=$o
    }
    function Set-ButtonIdle {
        $script:sync.Running    = $false
        $script:sync.StatusPill = "Idle"
    }

    function Set-AutoScroll {
        param([bool]$Enabled)
        $script:AutoScroll     = $Enabled
        $script:sync.AutoScroll = $Enabled
    ...
    function Open-WindowsUpdate {
        ...
        try {
            ...
            try { Start-Process "control" -ArgumentList "sysdm.cpl" -ErrorAction SilentlyContinue }
            catch { Log "WARNING: Could not open system settings - $($_.Exception.Message)" }
        }
    }
    ...
    function Play-Sound {
        ...
        if ($script:Headless -or -not $script:sync.SoundEnabled) { return }
        ...
        try { (New-Object System.Media.SoundPlayer $wavFile).Play() } catch {}
    }
    ...
    # ===================================================================
    # BELOW: VERBATIM 1.13.1 lines 1101-5680 (Assert-Curl ... Start-Install)
    # Only mechanical change applied: $script:CancelRequested -> $sync.Cancel
    # ===================================================================
...
#
# Cancellation: $sync.Cancel checked every 700ms; when set, every
        ...
        default {
            ...
            if (($Vendor -eq "Dell" -or $Vendor -eq "HP") -and (Test-Path $script:7zExe)) {
                ...
                while (-not $sevenProc.HasExited) {
                    ...
                    Step-OverallSpinner
                    [System.Windows.Forms.Application]::DoEvents()
                    if ($sync.Cancel) { try { $sevenProc.Kill() } catch {}; break }
                }
...
function Start-Install {
    ...
    # v1.13.1 - CANONICAL serial capture. Must happen here, before any
    # early-return path (no-missing-drivers skip, TestMode short-circuit,
    # pre-install failure), otherwise those rows reach the analytics
    # sheet with an empty serial. The per-vendor install functions still
    # re-capture this (same Win32_BIOS query) - now redundant but kept as
    # harmless defensive no-ops; this assignment is the source of truth.
    try { $script:AnalyticsSerial = (Get-CimInstance Win32_BIOS).SerialNumber.Trim() }
    catch { Log "  Could not read BIOS serial for analytics: $($_.Exception.Message)" -Level "warn" -Event "serial_read_error" }
    try {
    ...
    if ($sync.Cancel) { Send-AnalyticsEvent -Result "cancelled" }
    ...
    if ($success) {
        ...
        if (-not $sync.Cancel) { Send-AnalyticsEvent -Result "failure" }
        $reportResult = if ($sync.Cancel) { "cancelled" } else { "failure" }
    ...
    # ===================================================================
    # END VERBATIM PRESERVED BLOCK
    # ===================================================================

    try {
        Start-Install
    } catch {
        try { Log "FATAL: unhandled worker exception - $($_.Exception.Message)" -Level "error" -Event "worker_fatal" } catch {}
    } finally {
        $script:sync.Running  = $false
        $script:sync.Finished = $true
        $script:sync.StatusPill = "Done"
    }
}
...
# =============================================================
if ($Headless) {
    # Headless / -Silent: run the pipeline in-process exactly like 1.x.
    & $WorkerBody $sync $Ctx
    return
}

# ---- GUI MODE: WPF window on the main (STA) thread, pipeline on a
#      dedicated STA background runspace, ~90ms DispatcherTimer pump. ----

[xml]$Xaml = @'
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="Driver Installer" Height="660" Width="940"
        WindowStartupLocation="CenterScreen" Background="#0F1115"
        FontFamily="Segoe UI" ResizeMode="CanMinimize">
  <Window.Resources>
    <Style TargetType="ProgressBar">
      <Setter Property="Height" Value="10"/>
      <Setter Property="Foreground" Value="#5B8DEF"/>
  ...
  <Grid Margin="22">
    ...
    <Grid Grid.Row="0">
      ...
      <StackPanel Grid.Column="0">
        <TextBlock x:Name="TitleText" Text="Driver Installer" Foreground="White"
                   FontSize="22" FontWeight="Bold"/>
        <TextBlock x:Name="SubtitleText" Text="Detecting hardware..."
                   Foreground="#8B95A7" FontSize="13" Margin="0,2,0,0"/>
      </StackPanel>
      <Border Grid.Column="1" Style="{StaticResource Pill}" VerticalAlignment="Center">
        <StackPanel Orientation="Horizontal">
          <Ellipse x:Name="PillDot" Width="9" Height="9" Fill="#8B95A7"
                   VerticalAlignment="Center" Margin="0,0,7,0"/>
          <TextBlock x:Name="PillText" Text="Idle" Foreground="#C7CEDB"
    ...
    <!-- Progress card -->
    <Border Grid.Row="1" Background="#161A22" CornerRadius="12" Padding="20"
            Margin="0,18,0,0">
      <StackPanel>
        <DockPanel Margin="0,0,0,4">
          <TextBlock Text="Download" Foreground="#C7CEDB" FontWeight="SemiBold"/>
          <TextBlock x:Name="DlSpin" DockPanel.Dock="Right" Foreground="#5B8DEF"
                     FontFamily="Consolas" Margin="8,0,0,0"/>
          <TextBlock x:Name="DlLabel" DockPanel.Dock="Right" Foreground="#8B95A7"
                     HorizontalAlignment="Right" Text="Waiting..."/>
        </DockPanel>
        <ProgressBar x:Name="DlBar" Minimum="0" Maximum="100"/>

        <DockPanel Margin="0,16,0,4">
          <TextBlock x:Name="ExHeader" Text="Extract &amp; install"
                     Foreground="#C7CEDB" FontWeight="SemiBold"/>
          <TextBlock x:Name="ExSpin" DockPanel.Dock="Right" Foreground="#5B8DEF"
                     FontFamily="Consolas" Margin="8,0,0,0"/>
          <TextBlock x:Name="ExLabel" DockPanel.Dock="Right" Foreground="#8B95A7"
                     HorizontalAlignment="Right" Text="Waiting..."/>
        </DockPanel>
        <ProgressBar x:Name="ExBar" Minimum="0" Maximum="100"/>

        <DockPanel Margin="0,16,0,4">
          <TextBlock Text="Overall" Foreground="#C7CEDB" FontWeight="SemiBold"/>
          <TextBlock x:Name="OvSpin" DockPanel.Dock="Right" Foreground="#7A5BEF"
                     FontFamily="Consolas" Margin="8,0,0,0"/>
        </DockPanel>
        <ProgressBar x:Name="OvBar" Minimum="0" Maximum="100" Foreground="#7A5BEF"/>
      </StackPanel>
    </Border>

    <!-- Log -->
    <Border Grid.Row="2" Background="#0B0D11" CornerRadius="12" Margin="0,16,0,0"
            BorderBrush="#1C2230" BorderThickness="1">
      <TextBox x:Name="LogBox" Background="Transparent" Foreground="#9FD0A6"
               BorderThickness="0" IsReadOnly="True" FontFamily="Consolas"
               FontSize="12" Padding="14" TextWrapping="NoWrap"
    ...
    <Grid Grid.Row="3" Margin="0,16,0,0">
      ...
      <CheckBox x:Name="SoundChk" Grid.Column="0" Content="Sound" IsChecked="True"
                Foreground="#8B95A7" VerticalAlignment="Center" Margin="0,0,16,0"/>
      <CheckBox x:Name="ScrollChk" Grid.Column="1" Content="Auto-scroll" IsChecked="True"
                Foreground="#8B95A7" VerticalAlignment="Center"/>
      <Button x:Name="CancelBtn" Grid.Column="3" Content="Cancel" Width="120"
              Style="{StaticResource Btn}" Background="#3A3F4B" Margin="0,0,10,0"/>
      <Button x:Name="RunBtn" Grid.Column="4" Content="Run" Width="140"
              Style="{StaticResource Btn}" Background="#5B8DEF"/>
    </Grid>
  </Grid>
</Window>
'@
...
$UI = @{}
foreach ($n in 'TitleText','SubtitleText','PillDot','PillText','DlSpin','DlLabel',
                'DlBar','ExHeader','ExSpin','ExLabel','ExBar','OvSpin','OvBar',
                'LogBox','SoundChk','ScrollChk','CancelBtn','RunBtn') {
...
function Start-Worker {
    ...
    $script:rs.SessionStateProxy.SetVariable('Ctx',  $Ctx)
    $script:ps = [powershell]::Create()
    $script:ps.Runspace = $script:rs
    [void]$script:ps.AddScript($WorkerBody.ToString()).AddArgument($sync).AddArgument($Ctx)
    $script:async = $script:ps.BeginInvoke()
}

function Stop-WorkerResources {
...
# Dispatcher pump ---------------------------------------------------------
$dotBrush = @{
    idle = '#8B95A7'; work = '#5B8DEF'; ok = '#3FB950'; err = '#F85149'
}
$timer = New-Object System.Windows.Threading.DispatcherTimer
$timer.Interval = [TimeSpan]::FromMilliseconds(90)
$timer.Add_Tick({
    # Drain log queue
    if ($sync.LogQueue.Count -gt 0) {
        $sb = New-Object System.Text.StringBuilder
        while ($sync.LogQueue.Count -gt 0) {
            [void]$sb.AppendLine([string]$sync.LogQueue.Dequeue())
        }
        $UI.LogBox.AppendText($sb.ToString())
        if ($sync.AutoScroll) { $UI.LogBox.ScrollToEnd() }
    }

    $UI.TitleText.Text    = [string]$sync.Title
    $UI.SubtitleText.Text = [string]$sync.Subtitle
    $window.Title         = [string]$sync.Title

    $d = $sync.Dl
    $UI.DlLabel.Text = [string]$d.Label
    $UI.DlSpin.Text  = [string]$d.Spin
    $UI.DlBar.IsIndeterminate = ($d.Mode -eq 'marquee')
    if ($d.Mode -ne 'marquee') { $UI.DlBar.Value = [double]$d.Pct }

    $e = $sync.Ex
    $UI.ExLabel.Text  = [string]$e.Label
    $UI.ExSpin.Text   = [string]$e.Spin
    $UI.ExHeader.Text = [string]$sync.HeaderText
    $UI.ExBar.IsIndeterminate = ($e.Mode -eq 'marquee')
    if ($e.Mode -ne 'marquee') { $UI.ExBar.Value = [double]$e.Pct }

    $UI.OvSpin.Text = [string]$sync.Ov.Spin
    $UI.OvBar.Value = [double]$sync.Progress

    $pill = [string]$sync.StatusPill
    $UI.PillText.Text = $pill
    $pd = switch ($pill) { 'Running' {'work'} 'Done' {'ok'} default {'idle'} }
    $UI.PillDot.Fill  = $dotBrush[$pd]

    $UI.RunBtn.IsEnabled    = -not $sync.Running
    $UI.CancelBtn.IsEnabled = $sync.Running

    if ($sync.CloseWindow) { $sync.CloseWindow = $false; $window.Close() }

    if ($sync.Finished -and -not $sync.Running) {
        $sync.Finished = $false
        Stop-WorkerResources
    }
})

# Wire up -----------------------------------------------------------------
$UI.RunBtn.Add_Click({ Start-Worker })
...
$UI.SoundChk.Add_Click({  $sync.SoundEnabled = [bool]$UI.SoundChk.IsChecked })
$UI.ScrollChk.Add_Click({ $sync.AutoScroll   = [bool]$UI.ScrollChk.IsChecked })

$window.Add_Loaded({
    $timer.Start()
    Start-Worker          # auto-start, matching 1.x $form.Add_Shown behaviour
})
$window.Add_Closing({
    if ($sync.Running) { $sync.Cancel = $true }
})
$window.Add_Closed({
    $timer.Stop()
    if ($sync.Running -and $script:ps) {
